...
const Dashboard = () => {
  ...
  return(
    ...
    ? <div className="dashboard min-h-screen min-w-full bg-bg-1 flex-1 w-full flex flex-col md:flex-row">
          <div className="realtime-emotion flex flex-col items-center justify-center w-[400px] h-[300px] sm:w-[600px] sm:-h[400px] md:w-[700px] md:h-[450px] lg:w-[700px] lg:h-[600px]">
            <VideoComponent />
            <Settings />
          </div>
        ...
        <AnimatePresence
          ...
          onExitComplete={() => null}
        >
          {window.innerWidth >= 870 && recordedExpressionsVisible }
        </AnimatePresence>
      </div>
    : <span className="min-h-screen flex flex-col items-center justify-center bg-bg-1">
        <Spinner text={"Chargement des modèles"} />
      </span>
  );
};
